...
use super::keyboard::{
    vk_to_scan_code, send_key_game_only_down, send_key_game_only_up,
    send_key_normal_down, send_key_normal_up,
};
...
impl AutoFireEngine {
    ...
    pub fn start(&mut self) {
        ...
        let configured_keys = self.configured_keys.clone();
        let pressed_keys = self.pressed_keys.clone();
        let stop_signal = self.stop_signal.clone();

        let hook_handle = thread::spawn(move || {
            run_keyboard_hook(enabled.clone(), configured_keys, pressed_keys, stop_signal);
        });
        ...
        let handle = thread::spawn(move || {
            autofire_loop(enabled, configured_keys, pressed_keys, stop_signal);
        });
...
fn run_keyboard_hook(
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        SetWindowsHookExW, UnhookWindowsHookEx, GetMessageW, WH_KEYBOARD_LL, MSG,
    };
...
static HOOK_ENABLED_FLAG: once_cell::sync::Lazy<RwLock<Option<Arc<AtomicBool>>>> =
    once_cell::sync::Lazy::new(|| RwLock::new(None));
static HOOK_CONFIGURED_KEYS_REF: once_cell::sync::Lazy<RwLock<Option<Arc<RwLock<HashSet<u16>>>>>> =
    once_cell::sync::Lazy::new(|| RwLock::new(None));

const WM_KEYDOWN: u32 = 0x0100;
const WM_KEYUP: u32 = 0x0101;

/// 将 VK 码转换为位掩码索引（使用 VK 码本身作为位索引）
fn vk_to_bit(vk: u16) -> u64 {
    if vk < 64 {
        1u64 << vk
    } else {
        // 对于 VK >= 64，使用 hash 映射到 0-63 范围
        1u64 << (vk % 64)
    }
}
...
unsafe extern "system" fn keyboard_hook_proc(
    ...
    if code >= 0 && HOOK_ENABLED.load(Ordering::SeqCst) {
        ...
        if !is_injected {
            ...
            let is_engine_enabled = enabled_flag.as_ref()
            ...
            if is_engine_enabled {
                ...
                let is_configured = configured_ref.as_ref()
    ...
    loop {
        ...
        if !is_dnf_active() {
            ...
            if now - last >= 3 {
                ...
                let class = super::window::get_foreground_class_name();
